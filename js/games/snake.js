// Snake game implementation for a dedicated game window
...
export function setupSnakeGameWindow(windowElement) {
  const grid = windowElement.querySelector(".snake-grid");
  const scoreEl = windowElement.querySelector(".snake-score");
  const statusEl = windowElement.querySelector(".snake-status");
  const restartBtn = windowElement.querySelector(".snake-restart-btn");

  if (!grid || !scoreEl || !statusEl || !restartBtn) return;

  const rows = parseInt(grid.dataset.rows || "15", 10);
  const cols = parseInt(grid.dataset.cols || "20", 10);
